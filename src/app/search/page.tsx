...
import { MOCK_SEARCH_SHOWS } from "@/lib/searchMockData";
import { filterShows } from "@/lib/filterUtils";
...
const CURATOR_PICKS = MOCK_SEARCH_SHOWS.filter((s) => s.isCuratorPick).slice(0, 4);

// 활성 모드 태그 (지도/장르 선택 시 상단에 표시)
...
export default function SearchPage() {
  ...
  const keywordShows = useMemo(() => {
    if (!keyword.trim()) return [];
    return filterShows(MOCK_SEARCH_SHOWS, {
      keyword, region: null, venueId: null,
      genre: null, subGenre: null, extraTags: [],
    });
  }, [keyword]);
  ...
  return (
    <div className="min-h-screen lg:ml-[240px]" style={{ background: "#0c0c0c", color: "#ffffff" }}>
        ...
        {isKeywordMode ? (
          <div className="px-5">
            <div className="flex items-baseline gap-2 mb-4">
              ...
              <span className="text-[11px]" style={{ color: "rgba(255,255,255,0.38)" }}>{keywordShows.length}개</span>
            </div>
            {keywordShows.length > 0 ? (
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                {keywordShows.map((s) => <PosterCard key={s.id} show={s} />)}
              </div>
            ) : (
              <div className="rounded-2xl py-10 text-center"
                style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <p className="text-3xl mb-3">🔍</p>
                <p className="text-[13px] font-semibold mb-1" style={{ color: "rgba(255,255,255,0.6)" }}>조건에 맞는 공연이 없어요</p>
            ...
            >
              <div className="px-5">
                <div className="flex items-center gap-2 mb-3.5">
                  ...
                  <span className="text-[13px] font-bold" style={{ color: "rgba(255,255,255,0.65)" }}>큐레이터 추천 공연</span>
                </div>
                <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
                  {CURATOR_PICKS.map((s) => <PosterCard key={s.id} show={s} />)}
                </div>
              </div>
            </div>
          </>
